...
Classmate 是一个基于 Flutter 的课程表应用，支持多课表管理、节次时间集复用、主题设置、课表显示与交互设置、学校站点管理，以及通过学校网页或 HTML 源码导入课表。
...
- 课表显示与交互：支持课程弹窗关闭方式、保留空白时间、已结束课程 / 之后课程显示，以及课表网格线开关
...
项目提供了 PHP 中转接口：[web/api.php](web/api.php)。应用内学校网页导入接口地址由 [lib/config/app_config.dart](lib/config/app_config.dart) 读取。
